...
let rec expression fmt e = 
  ...
  | App (Prim IfZ, Tuple [ cond; lhs; rhs ]) ->
      Format.fprintf fmt "if %a then@, @[%a@]@, else@, @[%a@]" 
        ...
        expression lhs
        expression rhs
  | App (Prim ((Mul | Div | Add | Sub | AddFloat | SubFloat | MulFloat | DivFloat | LessThanInt | LessThanFloat) as p), Tuple [ lhs; rhs ]) -> 
...
and ty fmt = function
  ...
  | TyArray t -> Format.fprintf fmt "array (%a)" ty t
  | TyRecursive (lhs, rhs) ->
     Format.fprintf fmt "%a -[rec]-> %a" 
